 ...
 *  2.AOF
 *      a.每隔一段时间（默认每一秒）将执行写命令append写入文件，启动时会根据日志从头到尾全部执行一遍以完成数据的恢复工作。包括flushDB也会执行
 *      b.有两种方式触发：有写操作就写、每秒定时写（也会丢数据）
 *      c.随着redis一直运行，aof文件会变的很大，可通过BGREWRITEAOF命令或者设置参数redis会启动aof文件缩减命令
 *          1.里面的有些多条指令可以合并
